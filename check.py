...
def run_order(hostname, port, username, password):
    ...
    if c_pid  == False and t_pid  == False and s_pid == False:
        ...
        print("The equipment with operating problems.")
        model = 'a'
        ...
        if model == 'm':
                    ...
                    if index == 0 and "capture.py -b" in line:
                        ...
                        for item in line:
                            if item.isdigit():
                                stdin, stdout, stderr = client.exec_command("kill -9 " + str(item))
                                break
            # run nohup capture.py -b enp4s1 -s ip &
            stdin, stdout, stderr = client.exec_command("cd /root/capture_traffic; nohup python3 capture.py -b enp4s1 -s {} &".format(hostname))
            print("cd /root/capture_traffic; nohup python3 capture.py -b ens33 -s {} &".format(hostname))
            time.sleep(1)
            ...
            stdin, stdout, stderr = client.exec_command("tail /dev/shm/logs/ready/net_alert/alert.log")
            print('\n')
            # 读取命令结果
            for line in stdout:
                line = line.strip('\n')
                print(line)
            time.sleep(1)
            stdin, stdout, stderr = client.exec_command("tail /dev/shm/logs/ready/net_status/tshark_performance.log")
            print('\n')
            # 读取命令结果
            for line in stdout:
                line = line.strip('\n')
...
while True:
    ...
    count = 0
    time.sleep(5)
    flag = input("Do you want to continue checking?(yes/no)")
    if flag == "no":
        quit()
    else:
        pass
